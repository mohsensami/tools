...
const Dictunary = () => {
  ...
  useEffect(() => {
    console.log(dictionaryResults?.data[0]);
  }, []);
  return (
    <div>
      <div
      ...
      <div className="max-w-2xl mx-auto p-6 bg-white shadow-lg rounded-lg">
        <h1 className="text-3xl font-bold text-gray-900 mb-4">
          {dictionaryResults?.data?.word}
        </h1>
        <div className="space-y-2">
          {dictionaryResults?.data?.phonetics.map((phonetic, index) => (
            <div key={index} className="flex items-center space-x-2">
              {phonetic.text && (
                <span className="text-gray-700">{phonetic.text}</span>
              )}
              {phonetic.audio && (
        ...
        <div className="mt-4">
          {dictionaryResults?.data?.meanings.map((meaning, index) => (
        ...
        <div className="mt-4 text-sm text-gray-500">
          <p>
            Source:{" "}
            <a
              href={dictionaryResults?.data?.sourceUrls[0]}
              className="text-blue-500 underline"
              target="_blank"
              rel="noopener noreferrer"
            >
              {dictionaryResults?.data?.sourceUrls[0]}
            </a>
          </p>
        </div>
      </div>
